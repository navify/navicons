...
import { getSvgContent, naviconContent } from './request';
...
export class Icon {
   ...
   */
  @Prop({ mutable: true }) mode = getNavMode();
  ...
  /**
   * If enabled, nav-icon will be loaded lazily when it's visible in the viewport.
   * Default, `false`.
   */
  @Prop() lazy = false;
  ...
  loadIcon() {
    if (Build.isBrowser && this.isVisible) {
      ...
      if (url) {
        if (naviconContent.has(url)) {
          // sync if it's already loaded
          this.svgContent = naviconContent.get(url);
          ...
          getSvgContent(url, this.sanitize).then(() => (this.svgContent = naviconContent.get(url)));
        }
      }
    }
