...
async function collectionCopy(rootDir: string) {
  ...
  const optimizedSrc = join(rootDir, 'dist', 'navicons', 'svg');
  ...
  const cePackageJson = {
    name: 'navicons/components',
    description: 'Navicons custom element.',
    main: './index.cjs.js',
    module: './index.js',
    types: './index.d.ts',
    private: true,
  ...
  /**
   * TODO: Remove this in Navicons v6.0
   * Rindo 2 removed the legacy loader,
   * but that is what Navicons users were using
   * to load Navicons from a CDN. The lines
  ...
  const installLoaderDest = join(rootDir, 'dist', 'navicons.js');
