...
async function build(rootDir: string) {
  try {
    ...
    const distNaviconsDir = join(distDir, 'navicons');
    const distSvgDir = join(distDir, 'svg');

    await Promise.all([fs.emptyDir(iconDir), fs.emptyDir(distDir)]);
    await fs.emptyDir(distSvgDir), await fs.emptyDir(distNaviconsDir);

    const pkgData = await fs.readJson(pkgJsonPath);
    const version = pkgData.version as string;

    const srcSvgData = await getSvgs(srcSvgDir, distSvgDir, distNaviconsDir);
          ...
          fn: (item, params) => {
            ...
            if (item.isElem()) {
              item.eachAttr((attr) => {
                if (attr.name === 'fill') {
                  if (attr.value === 'none') {
                    item.class.add('navicon-fill-none');
                  }
                  ...
                  item.class.add('navicon-stroke-width');
                }
              });
            }
          },
...
async function optimizeSvg(
  ...
  const optimizedCode = optimizedSvg.data.replace(
    /<svg (.*?)>/,
    `<svg xmlns="http://www.w3.org/2000/svg" class="navicon" viewBox="0 0 512 512"><title>${svgData.title}</title>`,
  );
...
async function createSvgSymbols(version: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const symbolsSvgFilePath = join(distDir, 'navicons.symbols.svg');

  const lines = [
    `<svg data-navicons="${version}" style="display:none">`,
    `<style>`,
    `.navicon {`,
    `  fill: currentColor;`,
    `  stroke: currentColor;`,
...
async function getSvgs(srcSvgDir: string, distSvgDir: string, distNaviconsDir: string): Promise<SvgData[]> {
  const optimizedSvgDir = join(distNaviconsDir, 'svg');
  ...
  const svgData = await Promise.all(
    svgFiles.map(async (fileName) => {
      ...
      // optimizedFilePath: /dist/navicons/svg/airplane-outline.svg
...
async function createEsmIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Navicons v${version}, ES Modules */`, ``];
...
async function createCjsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Navicons v${version}, CommonJS */`, ``];
...
async function createDtsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Navicons v${version}, Types */`, ``];
...
async function createDataJson(version: string, srcDir: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const distDataJsonPath = join(distDir, 'navicons.json');
...
interface SvgData {
  ...
  /**
   * /dist/navicons/svg/airplane-outline.svg
   */
